...
namespace Appccelerate.StateMachine.Machine
{
    ...
    where TEvent : IComparable
  {
    ...
    public void Fire(TEvent eventId, object eventArgument)
    {
      ...
      ITransitionContext<TState, TEvent> context = this.factory.CreateTransitionContext(this.CurrentState,
        new Missable<TEvent>(eventId), eventArgument, this);
    ...
    public void Save(IStateMachineWriter<TState> stateMachineWriter)
    {
      Ensure.ArgumentNotNull(stateMachineWriter, "stateMachineSaver");

      stateMachineWriter.SaveCurrentState(this.currentState != null
      ...
      stateMachineWriter.SaveHistoryStates(historyStates);
    }

    public void Load(IStateMachineReader<TState> stateMachineReader)
    {
      Ensure.ArgumentNotNull(stateMachineReader, "stateMachineLoader");
      this.CheckThatStateMachineIsNotAlreadyInitialized();

      this.LoadCurrentState(stateMachineReader);
      this.LoadHistoryStates(stateMachineReader);
    }
    ...
    private void OnTransitionCompleted(ITransitionContext<TState, TEvent> transitionContext)
    {
      this.RaiseEvent(this.TransitionCompleted,
        new TransitionCompletedEventArgs<TState, TEvent>(this.CurrentStateId, transitionContext), transitionContext,
        true);
    }

    private void LoadCurrentState(IStateMachineReader<TState> stateMachineReader)
    {
      Initializable<TState> loadedCurrentState = stateMachineReader.LoadCurrentState();

      this.currentState = loadedCurrentState.IsInitialized ? this.states[loadedCurrentState.Value] : null;
    }

    private void LoadHistoryStates(IStateMachineReader<TState> stateMachineReader)
    {
      IDictionary<TState, TState> historyStates = stateMachineReader.LoadHistoryStates();
