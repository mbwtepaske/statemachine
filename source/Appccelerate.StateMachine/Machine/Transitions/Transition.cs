...
namespace Appccelerate.StateMachine.Machine.Transitions
{
    ...
    where TEvent : IComparable
  {
    ...
    {
      return string.Format(CultureInfo.InvariantCulture, "Transition from state {0} to state {1}.", this.Source,
        this.Target);
    }
    ...
    private bool ShouldFire(ITransitionContext<TState, TEvent> context)
    {
      ...
      catch (Exception exception)
      {
        this.extensionHost.ForEach(
          extention => extention.HandlingGuardException(this.stateMachineInformation, this, context, ref exception));

        HandleException(exception, context);

        this.extensionHost.ForEach(
          extention => extention.HandledGuardException(this.stateMachineInformation, this, context, exception));
    ...
    private void PerformActions(ITransitionContext<TState, TEvent> context)
    {
      foreach (IActionHolder action in this.actions)
      {
        ...
        catch (Exception exception)
        {
          this.extensionHost.ForEach(
            extension =>
              extension.HandlingTransitionException(this.stateMachineInformation, this, context, ref exception));

          HandleException(exception, context);

          this.extensionHost.ForEach(
            extension => extension.HandledTransitionException(this.stateMachineInformation, this, context, exception));
        }
      }
    }
